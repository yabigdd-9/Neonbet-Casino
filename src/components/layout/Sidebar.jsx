...
import { Gamepad2, Dice5, Sparkles, CircleDollarSign, Gift, ShieldCheck, Wallet, Trophy, ShieldCheck as Shield } from "lucide-react";
...
export default function Sidebar({ open, setOpen }) {
  return (
    <>
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 border-r border-white/10 bg-slate-950/95 p-5 backdrop-blur-xl transition-transform lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="grid h-11 w-11 place-items-center rounded-2xl bg-cyan-400 font-black text-slate-950 shadow-neon">{brand.shortName}</div>
            <div>
        ...
        <nav className="space-y-2">
          {LINKS.map(([label, Icon, sectionId]) => (
            <button
              key={label}
              type="button"
              onClick={() => { scrollToSection(sectionId); setOpen(false); }}
        ...
        <div className="absolute bottom-5 left-5 right-5 rounded-3xl border border-white/10 bg-white/5 p-4">
          <div className="flex items-center gap-2 font-bold text-amber-300"><Shield size={18} /> Account Notice</div>
          <p className="mt-2 text-xs text-slate-400">Bonuses and verification are subject to approval, local rules, and account terms.</p>
        </div>
      </aside>
