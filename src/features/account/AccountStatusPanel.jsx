...
export default function AccountStatusPanel({ user, profile, latestSubmission }) {
  if (!user) return null;
    ...
    <section id="account-status" className="scroll-mt-24 rounded-[2rem] border border-white/10 bg-white/[0.06] p-6 md:p-8">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-black md:text-3xl">{profile?.username || user.username || user.email}</h2>
          <p className="mt-2 text-slate-400">
            {IS_SUPABASE ? "Synced with Supabase accounts and verification records." : "Local browser account. Add Supabase env vars for real backend accounts."}
          </p>
        </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <div className="rounded-3xl border border-white/10 bg-black/20 p-5">
          ...
          <div className="mt-1 text-3xl font-black">{formatMoney(profile?.bonus_balance ?? 100)}</div>
        </div>
        <div className="rounded-3xl border border-white/10 bg-black/20 p-5">
          ...
          <p className="mt-2 text-xs text-slate-500">{formatMoney(progress)} / {formatMoney(required)}</p>
        </div>
        <div className="rounded-3xl border border-white/10 bg-black/20 p-5">
