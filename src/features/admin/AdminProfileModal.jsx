...
export default function AdminProfileModal({ userProfile, onClose, onSubmit, saving }) {
  ...
  const [rolloverProgress, setRolloverProgress] = useState(
    String(userProfile?.rollover_progress ?? 0),
  );
  const [rolloverRequired, setRolloverRequired] = useState(
    String(userProfile?.rollover_required ?? 1000),
  );
  ...
  return (
    <Modal
      title={userProfile.username || userProfile.email}
    ...
    >
      ...
      >
        <p className="text-sm text-slate-400">
          Edit bonus balance, rollover progress, and admin notes.
        </p>
