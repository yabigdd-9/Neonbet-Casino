...
export default function ArcadeGameModal({
  ...
  function finishRound(nextRound: RoundState) {
  ...
  function playRound() {
    ...
    window.setTimeout(() => {
      if (currentGame.id === "dice-duel") playDice();
      if (currentGame.id === "plinko-drop") playPlinko();
      if (currentGame.id === "neon-crash") playCrash();
    }, 550);
  }
                ...
                <div className="mt-4 grid grid-cols-9 gap-2 text-center text-xs font-black">
                  {["0x", ".4x", ".8x", "1.5x", "9x", "1.5x", ".8x", ".4x", "0x"].map(
                    (label, index) => (
                      <div
                        key={`${label}-${index}`}
                        className="rounded-xl bg-white/10 py-2 text-center text-cyan-200"
                      >
                        {label}
                      </div>
                    ),
