...
export default function ArcadeGamesSection({ onPlay }: ArcadeGamesSectionProps) {
  return (
    <section
      id="arcade-games"
      className="scroll-mt-24 rounded-[2rem] border border-cyan-300/20 bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950/60 p-5 md:p-7"
    >
      <div className="mb-5 flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-slate-400">
            Three fast browser games now sit beside the slot lobby, all sharing the same simulated
            balance and bet controls.
          </p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm font-bold text-slate-300">
          Dice, Plinko and Crash are local demo games only.
        </div>
      </div>
      <div className="grid gap-5 lg:grid-cols-3">
        {arcadeGames.map((game) => (
          <article
            key={game.id}
            className="group relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 transition hover:-translate-y-1 hover:border-cyan-300/40 hover:shadow-neon"
          >
            <div
              className={`absolute inset-x-0 top-0 h-28 bg-gradient-to-br ${game.gradient} opacity-90`}
            />
            <div className="relative">
              ...
              <div className="mt-10 text-xs font-bold uppercase tracking-[0.18em] text-cyan-300">
                {game.type}
              </div>
