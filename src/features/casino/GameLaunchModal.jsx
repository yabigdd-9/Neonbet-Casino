...
import { betOptions, getGameId } from "../../lib/gameEngine";
  ...
  return (
    <Modal onClose={onClose} maxWidth="max-w-4xl">
      <div className={`relative grid h-44 place-items-center bg-gradient-to-br ${game.gradient}`}>
        <button type="button" onClick={onClose} className="absolute right-4 top-4 rounded-2xl bg-black/30 p-3 text-white hover:bg-black/45" aria-label="Close">
          ✕
        </button>
      ...
      <div className="grid gap-6 p-6 md:grid-cols-[1.2fr_0.8fr] md:p-8">
        <div>
          ...
          <p className="mt-4 text-slate-400">Launch a fast simulated session with NeonBet balance controls, frequent local wins, and quick bet presets.</p>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-5">
          ...
          >
            <Star size={18} className={isFavorite ? "fill-amber-300 text-amber-300" : ""} /> {isFavorite ? "Remove favorite" : "Add favorite"}
          </button>
          <p className="mt-4 text-xs leading-5 text-slate-500">Simulated browser game only. Not connected to deposits, withdrawals, or provider servers.</p>
        </div>
      </div>
    </Modal>
  );
