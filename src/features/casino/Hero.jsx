...
import { games as featuredGames } from "../../data/games";
import { arcadeGames } from "../../data/arcadeGames";

export default function Hero({ onClaim, onOpenGame }) {
  return (
    <section id="lobby" className="relative scroll-mt-24 overflow-hidden rounded-[2rem] border border-cyan-300/20 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-950 p-6 shadow-neon md:p-10">
      ...
      <div className="relative grid items-center gap-8 lg:grid-cols-2">
        <div>
          ...
          <p className="mt-5 max-w-xl text-slate-300">Claim a free $100 sign-up bonus, unlock a 300% welcome match and clear a 10x rollover requirement.</p>
          <div className="mt-7 flex flex-wrap gap-3">
            <button type="button" onClick={onClaim} className="flex items-center gap-2 rounded-2xl bg-cyan-400 px-6 py-4 font-black text-slate-950 shadow-neon">
              <Play size={18} /> Claim $100
            </button>
            <button
              type="button"
              onClick={() => document.getElementById("promotions")?.scrollIntoView({ behavior: "smooth", block: "start" })}
            ...
            <button
              type="button"
              onClick={() => document.getElementById("arcade-games")?.scrollIntoView({ behavior: "smooth", block: "start" })}
          ...
          <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[["$100", "Sign-up bonus"], ["10x", "Bonus rollover"], ["24+", "Slot providers"], ["$75", "Verification"]].map(([value, label]) => (
          ...
          <div className="rounded-[2rem] border border-white/10 bg-black/30 p-5">
            <div className="grid grid-cols-3 gap-3 text-center">
              {["7", "🍒", "💎", "BAR", "⭐", "777", "🍋", "🔥", "X"].map((item) => (
                <div key={item} className="grid aspect-square place-items-center rounded-3xl border border-white/10 bg-white/10 text-3xl font-black">
                  {item}
                </div>
              ))}
            </div>
