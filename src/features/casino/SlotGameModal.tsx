...
interface SlotGameModalProps {
  game: SlotGame | null;
  balance: number;
  setBalance: (updater: (current: number) => number) => void;
  onClose: () => void;
}

const DEFAULT_SYMBOLS = ["🍒", "⭐", "💎", "7", "🍋"];

export default function SlotGameModal({ game, balance, setBalance, onClose }: SlotGameModalProps) {
  const [bet, setBet] = useState(0.5);
  const [reels, setReels] = useState<string[]>(game?.symbols?.slice(0, 5) || DEFAULT_SYMBOLS);
  const [displayReels, setDisplayReels] = useState<string[] | null>(null);
  const [result, setResult] = useState<{ label: string; win: number; multiplier: number }>({
    label: "Ready",
    win: 0,
    multiplier: 0,
  });
  ...
  function randomSymbols(count: number): string[] {
    const symbols = currentGame.symbols || DEFAULT_SYMBOLS;
  ...
  function spin() {
    ...
    setBalance((c) => Number((c - bet).toFixed(2)));

    window.setTimeout(() => {
      const spinResult = buildSpin(currentGame);
