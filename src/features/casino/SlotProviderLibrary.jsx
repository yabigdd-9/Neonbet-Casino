...
export default function SlotProviderLibrary({ onPlay, favorites, isFavorite, onToggleFavorite, recentGames }) {
  ...
  return (
    <section id="providers" className="scroll-mt-24">
        ...
        {providerSections.map((provider) => (
          <article key={provider.name} className="rounded-3xl border border-white/10 bg-white/[0.06] p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="rounded-2xl bg-black/25 px-3 py-2 text-sm font-black text-cyan-300">{provider.games.length}</div>
            </div>
            <div className="mt-5 flex flex-wrap gap-2">
              {provider.playableGames.map((game) => (
                ...
                >
                  <Play size={12} />
                  {isFavorite?.(game) && <Star size={12} className="fill-amber-300 text-amber-300" />}
                  {game.title}
                </button>
              ))}
            </div>
      ...
      {providerSections.length === 0 && (
        <EmptyState title="No games match that search and filter." description="Try a different keyword or filter." />
      )}
    </section>
  );
}
