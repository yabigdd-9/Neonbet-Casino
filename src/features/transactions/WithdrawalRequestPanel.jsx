...
export default function WithdrawalRequestPanel({ user, profile, latestWithdrawal, withdrawalSaving, onSubmitWithdrawal }) {
  ...
  function handleSubmit(event) {
    ...
    if (!canSubmit) return;
    onSubmitWithdrawal({ amount_usd: Number(amountUsd), payout_method: payoutMethod, payout_address: payoutAddress.trim() });
    setAmountUsd("");
    setPayoutAddress("");
  }
    ...
    <section id="withdrawals" className="scroll-mt-24 rounded-[2rem] border border-white/10 bg-white/[0.06] p-6 md:p-8">
      <div className="mb-5 flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-slate-400">Submit a manual request after verification and rollover review. Admin approval is required before any payout.</p>
        </div>
        {latestWithdrawal && <StatusBadge status={latestWithdrawal.status} label={`Latest: ${latestWithdrawal.status}`} />}
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <div className="rounded-3xl border border-white/10 bg-black/20 p-5">
          ...
          <p className="mt-4 text-xs leading-5 text-slate-500">Requests can be submitted before final approval, but admin may reject requests that do not meet verification, rollover, or account review rules.</p>
        </div>

        <form onSubmit={handleSubmit} className="rounded-3xl border border-white/10 bg-black/20 p-5">
          <div className="grid gap-4 md:grid-cols-2">
            <Input label="Amount USD" type="number" min="1" step="0.01" value={amountUsd} onChange={(e) => setAmountUsd(e.target.value)} placeholder="100.00" />
          ...
          <div className="mt-4">
            <Input label="Payout wallet/address" value={payoutAddress} onChange={(e) => setPayoutAddress(e.target.value)} placeholder="Paste payout address" className="font-mono text-sm" />
          </div>
